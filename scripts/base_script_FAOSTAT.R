...
#As far as I can tell, FAO still stubbornly refuses to provide an API to allow us to scrape data directly.
...
FAOF <- subset(FAO, select = c('Area', 'Area.Code..ISO3.', 'Element', 'Year', 'Value'))
FAOF
...
FAOwide$continent <- countrycode(sourcevar = FAOwide[, "Area.Code..ISO3."],
                            origin = "iso3c",
                            destination = "continent")


FAOwide$region <- countrycode(sourcevar = FAOwide[, "Area.Code..ISO3."],
                                 origin = "iso3c",
                                 destination = "region")
...
FAOwide<-FAOwide%>%
  mutate(continent=ifelse(Country == 'China, mainland', 'Asia', continent)) %>% mutate(region=ifelse(Country == 'China, mainland', 'East Asia & Pacific', region))

#The entry for Sudan lacks both continent and region.
FAOwide<-FAOwide%>%
  mutate(continent=ifelse(Country == 'Sudan (former)', 'Africa', continent)) %>% mutate(region=ifelse(Country == 'Sudan (former)', 'Sub-Saharan Africa', region))
...
#################### Things we can do with our data #########################
#### gganimate code for getting an animated GIF ####
p <- ggplot(FAOwide, aes(Yield/10000, Area/1000000, size = Production/1000000, color = continent, frame = Year)) +
  labs(x="Yield", y = "Area", size = "Production", color = 'Continent') + 
  geom_point() +
...
p2 <- ggplot(FAOwide, aes(Yield, Area , size = Production, color = continent, frame = Year, label=Country)) +
  labs(x="Yield (T/Ha)", y = "Area (Ha)", size = "Production (Tons)", color = 'Continent') + 
  geom_point() +
  scale_color_brewer(type = 'div', palette = 'Spectral')
...
p3 <- ggplot(FAOwide, aes(size = Production/1000000, color = continent, label=Country, alpha=0.5, frame = Year, Yield, Area/1000000)) +
  labs(x="Yield (T/Ha)", y = "Area (M Ha)", size="Production (M T)") + 
  geom_point() +
  scale_color_brewer(type = 'div', palette = 'Spectral', na.translate = F)
...
ggp <- ggplotly(p3, height = 600, width = 900) %>%
  ...
  layout(annotations = 
           list(x = 1, y = -0.4, text = "Source: FAOSTAT (2022). Visualization: Erik Delaquis, Alliance of Bioversity International and CIAT", 
  ...
  layout(title = list(text = "Global cassava trends", font=list(size=18, color="black")))
ggp
...
# Custom function for checking html widget code
...
ham<-partial_bundle(ggp)
htmlwidgets::saveWidget(ham, "outputs/FAOpartial.html")
widget_file_size(ham)

#### Once again, with region ####

p4 <- ggplot(FAOwide, aes( size = Production/1000000, color = region, label=Country, frame = Year, Yield, Area/1000000)) +
  labs(x="Yield (T/Ha)", y = "Area (M Ha)", size="Production (M T)") + 
